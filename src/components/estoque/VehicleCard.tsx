...
export function VehicleCard({ vehicle, onView, onEdit, onDelete }: VehicleCardProps) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow overflow-hidden">
      {/* Imagem */}
      <div className="relative h-48 bg-gray-200 overflow-hidden group cursor-pointer" onClick={() => onView(vehicle)}>
        <img
          src={firstImage}
        ...
        />
        {hasPromotion && (
          <div className="absolute top-2 right-2 bg-red-500 text-white px-2 py-1 rounded-md flex items-center gap-1 text-sm font-semibold">
            <FiTag size={14} />
            Promoção
          </div>
        )}
      ...
      <div className="p-4">
        ...
        <div className="mb-4">
          ...
          <p className={`font-bold text-xl ${hasPromotion ? 'text-red-600' : 'text-gray-900'}`}>
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-orange-500 text-white rounded-md hover:bg-orange-600 transition-colors"
          >
            <FiEye size={16} />
            Ver
          </button>
