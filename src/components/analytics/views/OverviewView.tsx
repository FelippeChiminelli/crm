...
export function OverviewView({ data, filters, onFiltersChange, formatCurrency, formatPeriod }: OverviewViewProps) {
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-gray-600 mt-1">
              Resumo das principais métricas do CRM
              <span className="ml-2 font-medium text-blue-600">
                • {formatPeriod(filters.period.start, filters.period.end)}
              </span>
            </p>
          </div>
      ...
      <div className="p-6 space-y-6">
        ...
        {stats && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <KPICard
              ...
              value={totalLost}
              subtitle={`${lostConversionRate}% do total`}
              icon={<ChartBarIcon className="w-6 h-6" />}
              color="red"
              loading={loading}
            />
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <KPICard
            ...
            value={data.firstResponseTime?.formatted || 'Sem dados'}
          ...
          <KPICard
            ...
            value={data.proactiveContactTime?.formatted || 'Sem dados'}
          ...
          <KPICard
            title="Taxa de Conversão (Venda)"
            value={`${salesConversionRate}%`}
            subtitle={`${totalSales} vendas / ${totalLeads} leads`}
            icon={<CheckCircleIcon className="w-6 h-6" />}
            color="green"
            loading={loading}
          />
          ...
          <KPICard
            title="Taxa de Conversão (Perda)"
            value={`${lostConversionRate}%`}
            subtitle={`${totalLost} perdidos / ${totalLeads} leads`}
            icon={<ChartBarIcon className="w-6 h-6" />}
            color="red"
            loading={loading}
          />
